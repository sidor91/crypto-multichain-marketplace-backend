...
export class WithdrawalService {
  ...
  async WithdrawalTokenForOrder(chain_id: string, event: OrderEvent) {
    try {
      ...
      if (existingOrder && findTx) {
        if (
          ...
          existingOrder.status === OrderStatusType.CANCELED
        ) {
          return await this.withdrawToken(event, findTx);
        }
      ...
      await this.createWithdrawToken(chain_id, event, existingOrder);
    } catch (error) {
      this.handleServiceError(event, error);
    }
  }

  private async withdrawToken(event: OrderEvent, findTx: Transaction) {
    try {
      const updateTx = { status: ETransactionStatus.WITHDRAWN };
  ...
  private async createWithdrawToken(
    chain_id: string,
    event: OrderEvent,
    existingOrder: Order,
  ) {
    try {
      const user = await this.userService.findOrCreate(
        event.returnValues.order.requester,
      );

      const createTx = {
        chain_id,
        tx_hash: event.transactionHash,
        amount: event.returnValues.order.isNFT
          ? 1n
          : event.returnValues.order.initialTokens -
            event.returnValues.order.fulfilledToken,
        orderId: existingOrder.id,
        status: ETransactionStatus.WITHDRAWN,
      };
