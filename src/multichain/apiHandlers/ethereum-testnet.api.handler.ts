...
@Injectable()
export class EthereumTestNetApiHandler implements ApiHandlerInterface {
  constructor(
  ...
  public getHandlerName() {
    return EthereumTestNetApiHandler.name;
  }
  ...
  public async getTokenCirculatingSupply(
    ...
    const { data } = await lastValueFrom(
      this.httpService.get(
        `${api_endpoint}?module=stats&action=tokensupply&contractaddress=${options.address}&apikey=${api_key}`,
      ),
    );
    return data.result;
  }
